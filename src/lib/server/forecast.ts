...
/**
 * Generate a financial forecast based on:
  ...
  try {
    ...
    if (activeRecurrings.length > 0) {
      const fxAt = await resolveFxRates(
        supabase,
        activeRecurrings
          .filter((rec) => rec.base_amount == null)
          .map((rec) => ({ date: todayStr, from: rec.currency })),
        baseCurrency,
      );
      for (const rec of activeRecurrings) {
        ...
        if (baseAmt == null) {
          ...
          if (rec.currency === baseCurrency) {
            ...
            const rate = fxAt(todayStr, rec.currency);
            if (rate == null) continue; // skip rather than project a wrong number
            baseAmt = rawAmt * rate;
          }
        }
